/*
* Contains functions needed to generate exact polynomial
* features on GPU.
*/
...
//Generates the features for the exact quadratic.
template <typename T>
__global__ void genInteractionOnlyFeatures(T inArray[], double *outArray,
        int inDim1, int outDim1, int numElements){
    int pos = blockDim.x * blockIdx.x + threadIdx.x;
    int rowNum = pos / inDim1;
    int positionInRow = pos % inDim1;
    ...
    if (pos < numElements){
        *outPtr = inVal1;
        outPtr++;
        inPtr++;
        for (int i=positionInRow+1; i < inDim1; i++){
            *outPtr = inVal1 * *inPtr;
            outPtr++;
            inPtr++;
        }
...
//Performs feature generation for an exact quadratic (i.e. polynomial
//regression that is exact, not approximated) where interaction terms only
//are desired, i.e. x**2 terms are not generated.
//
//Note that all of these arrays are already expected to "live" on GPU.
template <typename T>
const char *cudaInteractionsOnly_(T inArray[], double *outArray,
    ...
    //Multiply by D1.
    genInteractionOnlyFeatures<T><<<blocksPerGrid, DEFAULT_THREADS_PER_BLOCK>>>(inArray, outArray, 
                                 inDim1, outDim1, numElements);
...
//Instantiate templates explicitly so wrapper can use.
template const char *cudaInteractionsOnly_<float>(float inArray[], double *outArray, 
                    int inDim0, int inDim1);
template const char *cudaInteractionsOnly_<double>(double inArray[], double *outArray, 
                    int inDim0, int inDim1);
