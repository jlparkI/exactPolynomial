...
/*!
 * # cpuInteractionsOnly_
 *
 * Generates the features for an exact quadratic if interaction terms
 * only are desired. The input
 * array is not changed and all features are written to the
 * designated output array.
 *
 * ## Args:
...
const char *cpuInteractionsOnly_(T inArray[], double *outArray,
    ...
    for (int i=0; i < numThreads; i++){
        ...
        if (endRow > inDim0)
            endRow = inDim0;
        threads[i] = std::thread(&ThreadInteractionsOnly<T>, inArray, outArray,
                                startRow, endRow, inDim0, inDim1);
    }
...
//Instantiate functions that the wrapper will need to use.
template const char *cpuInteractionsOnly_<double>(double inArray[], double *outArray,
                int inDim0, int inDim1, int numThreads);
template const char *cpuInteractionsOnly_<float>(float inArray[], double *outArray,
                int inDim0, int inDim1, int numThreads);
...
/*!
 * # ThreadInteractionsOnly
 *
 * Performs interaction only feature generation for one thread for a chunk of
 * the input array from startRow through endRow (each thread
 * works on its own group of rows).
 */
template <typename T>
void *ThreadInteractionsOnly(T inArray[], double *outArray, int startRow,
        int endRow, int inDim0, int inDim1){
    int numInteractions = inDim1 * (inDim1 - 1) / 2;
    int outDim1 = numInteractions + 1 + inDim1;
    T *inElement = inArray + startRow * inDim1;
    double *outElement = outArray + startRow * outDim1;
