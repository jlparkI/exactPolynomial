...
def initialize_srht_multipass(dataset, rank, kernel, random_state, verbose = False,
    ...
    if kernel.device == "cpu":
        acc_results = np.zeros((rank, kernel.get_num_feats()))
        ...
        if get_zty:
            z_trans_y, y_trans_y = np.zeros((kernel.get_num_feats())), 0.0
    else:
        mempool = cp.get_default_memory_pool()
        acc_results = cp.zeros((rank, kernel.get_num_feats()))
        ...
        if get_zty:
            z_trans_y, y_trans_y = cp.zeros((kernel.get_num_feats())), 0.0

    compressor = SRHTCompressor(rank, kernel.get_num_feats(),
...
def initialize_gauss(dataset, rank, kernel, random_state, verbose = False,
    ...
    Returns:
        ...
        y_trans_y: Either None or a float containing y^T y, depending on
            whether get_zty is True or False.
    """
    fitting_rffs = kernel.get_num_feats()
